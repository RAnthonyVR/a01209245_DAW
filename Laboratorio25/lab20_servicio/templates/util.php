<?php
  ...
  function insertFruit($name, $units, $quantity, $price, $country) {
        $bd = conectDb();
        //$sql="CALL insert_fruit ('$name','$units','$quantity','$price','$country')";
        $sql="CALL `insert_fruit` ('$name','$units','$quantity','$price','$country')";
        /*
        ...
        if(mysqli_query($conn,$sql)){
            ...
            echo "<div class=\"container\">ERROR: ".$sql."<br>".mysqli_error($conn)."</div>";
        }
        closeDb($conn);
    }
  ...
  function delete_by_name($fruit_name){
    ...
    $sql="CALL delete_fruit ('$fruit_name')";
  ...
  function update_by_name($name, $units, $quantity, $price, $country){
    ...
    //$sql="CALL update_fruit ('$name','$units','$quantity','$price','$country')";
    $sql='CALL `update_fruit` ('.$name.','.$units.','.$quantity.','.$price.','.$country.')';
    $result=mysqli_query($conn,$sql);
